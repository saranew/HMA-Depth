...
class ag(nn.Module):
    def __init__(self, channel):
        ...
        self.ag = nn.Sequential(
                nn.Conv2d(channel, 512, kernel_size=3,
                          padding=1, bias=False),
                nn.ReLU(inplace=True),
                nn.Conv2d(512, 1, kernel_size=1, padding=1,
                          bias=False))

        for model in self.ag:
...
class bts(nn.Module):
    def __init__(self, params, feat_out_channels, num_features=512):
        ...
        if self.params.model_ag:
            ...
            # self.ag2 = ag(128)
            # self.ag3 = ag(64)
            self.ag_conv  = torch.nn.Sequential(nn.Conv2d(num_features // 16, num_features // 16, 3, 1, 1, bias=False), nn.ELU())
            self.ag_conv1  = torch.nn.Sequential(nn.Conv2d(num_features // 16, num_features // 16, 3, 1, 1, bias=False), nn.ELU())
            self.ag_conv2  = torch.nn.Sequential(nn.Conv2d(num_features // 16, num_features // 16, 3, 1, 1, bias=False), nn.ELU())
            self.ag_conv3  = torch.nn.Sequential(nn.Conv2d(num_features // 16, num_features // 16, 3, 1, 1, bias=False), nn.ELU())
            self.ag_conv4  = torch.nn.Sequential(nn.Conv2d(num_features // 16, num_features // 16, 3, 1, 1, bias=False), nn.ELU())
            self.ag_get_depth = torch.nn.Sequential(nn.Conv2d(num_features // 16, num_features // 16, 3, 1, 1, bias=False),
                                                    nn.ELU(),
    ...
    def forward(self, features, focal):
        if self.params.model_ag:
            ...
            depth_8x8_scaled = self.head1(daspp_feat)
            ...
            depth_4x4_scaled = self.head2(iconv3)
            ...
            depth_2x2_scaled = self.head3(iconv2)
            
            upconv1 = self.upconv1(iconv2)
            reduc1x1 = self.head4(upconv1)
            # ag8 = nn.functional.interpolate(self.ag1(daspp_feat), size=reduc1x1[0,0].size(), mode='bilinear')
            # ag4 = nn.functional.interpolate(self.ag2(iconv3), size=reduc1x1[0,0].size(), mode='bilinear')
            # ag2 = nn.functional.interpolate(self.ag3(iconv2), size=reduc1x1[0,0].size(), mode='bilinear')
            depth_8x8_scaled = nn.functional.interpolate(depth_8x8_scaled, size=reduc1x1[0,0].size(), mode='bilinear')
            depth_4x4_scaled = nn.functional.interpolate(depth_4x4_scaled, size=reduc1x1[0,0].size(), mode='bilinear')
            depth_2x2_scaled = nn.functional.interpolate(depth_2x2_scaled, size=reduc1x1[0,0].size(), mode='bilinear')
            # reduc1x1 = self.ag_conv1(reduc1x1)
            # reduc1x1 = self.params.max_depth * self.ag_get_depth(reduc1x1)
            # depth_2x2_scaled = self.ag_conv2(depth_2x2_scaled)
            # depth_2x2_scaled = self.params.max_depth * self.ag_get_depth(depth_2x2_scaled)
            # depth_4x4_scaled = self.ag_conv3(depth_4x4_scaled)
            # depth_4x4_scaled = self.params.max_depth * self.ag_get_depth(depth_4x4_scaled)
            # depth_8x8_scaled = self.ag_conv4(depth_8x8_scaled)
            # depth_8x8_scaled = self.params.max_depth * self.ag_get_depth(depth_8x8_scaled)
            # ag1 = self.ag3(reduc1x1)
            # concat1 = reduc1x1*ag1 + depth_2x2_scaled*(1-ag1)*ag2 + depth_4x4_scaled*(1-ag1)*(1-ag2)*ag4 + depth_8x8_scaled*(1-ag1)*(1-ag2)*(1-ag4)
            # concat1 = torch.sigmoid(reduc1x1*(1-ag8)*(1-ag2)*(1-ag4) + depth_2x2_scaled*(1-ag8)*(1-ag4)*ag2 + depth_4x4_scaled*(1-ag8)*ag4 + depth_8x8_scaled*ag8)
            # concat1 = torch.cat([reduc1x1*(1-ag8)*(1-ag2)*(1-ag4), depth_2x2_scaled*(1-ag8)*(1-ag4)*ag2, depth_4x4_scaled*(1-ag8)*ag4, depth_8x8_scaled*ag8], dim=1)

            # mask1, mask2, mask3, mask4 = ag8, (1-ag8)*ag4, (1-ag8)*(1-ag4)*ag2, (1-ag8)*(1-ag2)*(1-ag4)

            final_depth = self.params.max_depth * (reduc1x1 + depth_2x2_scaled + depth_4x4_scaled + depth_8x8_scaled)
            # final_depth = self.params.max_depth * (reduc1x1*(1-ag8)*(1-ag2)*(1-ag4) + depth_2x2_scaled*(1-ag8)*(1-ag4)*ag2 + depth_4x4_scaled*(1-ag8)*ag4 + depth_8x8_scaled*ag8)
            # depth_8x8_scaled = self.params.max_depth * depth_8x8_scaled
            # depth_4x4_scaled = self.params.max_depth * depth_4x4_scaled
            # depth_2x2_scaled = self.params.max_depth * depth_2x2_scaled
            # reduc1x1 = self.params.max_depth * reduc1x1
            # final_depth = torch.cat([reduc1x1*(1-ag8)*(1-ag2)*(1-ag4) + depth_2x2_scaled*(1-ag8)*(1-ag4)*ag2 + depth_4x4_scaled*(1-ag8)*ag4 + depth_8x8_scaled*ag8], dim=1)
            # final_depth = self.ag_conv(final_depth)
            # final_depth = self.params.max_depth * self.ag_get_depth(final_depth)
            # final_depth = self.params.max_depth * concat1
            # concat1 = torch.cat([upconv1, concat1], dim=1)
            # final_depth = self.params.max_depth * self.ag_get_depth(concat1)
        else:
            skip0, skip1, skip2, skip3 = features[1], features[2], features[3], features[4]
            dense_features = torch.nn.ReLU()(features[5])
            upconv5 = self.upconv5(dense_features) # H/16
        ...
        if self.params.model_ag:
            return depth_8x8_scaled, depth_4x4_scaled, depth_2x2_scaled, reduc1x1, final_depth, final_depth, final_depth, final_depth, final_depth
        else:
...
class encoder(nn.Module):
    def __init__(self, params):
        ...
        if params.encoder == 'densenet121_bts':
            ...
            self.feat_names = ['relu', 'layer1', 'layer2', 'layer3', 'layer4']
            self.feat_out_channels = [64, 256, 512, 1024, 2048]
        elif params.encoder == 'resnest101e':
            self.base_model = create_model(params.encoder, pretrained=True, num_classes=1)
            self.feat_names = ['act1', 'layer1', 'layer2', 'layer3', 'layer4']
            self.feat_out_channels = [128, 256, 512, 1024, 2048]
        else:
