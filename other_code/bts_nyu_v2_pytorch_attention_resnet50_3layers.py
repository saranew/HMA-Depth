...
class bts(nn.Module):
    ...
    def forward(self, features, focal):
        if self.params.model_ag:
            ...
            size = depth_2x2_scaled[0,0].size()
            size = (size[0]*2, size[1]*2)
            ag8 = nn.functional.interpolate(self.ag1(daspp_feat), size=size, mode='bilinear')
            ag4 = nn.functional.interpolate(self.ag2(iconv3), size=size, mode='bilinear')
            # ag2 = nn.functional.interpolate(self.ag3(iconv2), size=reduc1x1[0,0].size(), mode='bilinear')
            depth_8x8_scaled = nn.functional.interpolate(depth_8x8_scaled, size=size, mode='bilinear')
            depth_4x4_scaled = nn.functional.interpolate(depth_4x4_scaled, size=size, mode='bilinear')
            depth_2x2_scaled = nn.functional.interpolate(depth_2x2_scaled, size=size, mode='bilinear')
            ...
            mask1, mask2, mask3 = ag8, (1-ag8)*ag4, (1-ag8)*(1-ag4)

            final_depth = self.params.max_depth * (depth_2x2_scaled*mask3 + depth_4x4_scaled*mask2 + depth_8x8_scaled*mask1)
        ...
        if self.params.model_ag:
            return depth_8x8_scaled, depth_4x4_scaled, depth_2x2_scaled, depth_2x2_scaled, final_depth, mask1, mask2, mask3, mask3
        else:
