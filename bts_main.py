...
def normalize_result(value, vmin=None, vmax=None):
    ...
    if vmin != vmax:
        value = (value - vmin) / (vmax - vmin + 0.000001)
    else:
        value = value * 0.
...
def online_eval(model, dataloader_eval, gpu, ngpus):
    ...
    for _, eval_sample_batched in enumerate(tqdm(dataloader_eval.data)):
        with torch.no_grad():
            ...
            _, _, _, _, pred_depth = model(image, focal)[:5]
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    while epoch < args.num_epochs:
        ...
        for step, sample_batched in enumerate(dataloader.data):
            ...
            lpg8x8, lpg4x4, lpg2x2, reduc1x1, depth_est = model(image, focal)[:5]
            ...
            # if args.model_ag:
            #     loss = loss * 4
            #     loss += silog_criterion.forward(lpg8x8, depth_gt, mask.to(torch.bool))
            #     loss += silog_criterion.forward(lpg4x4, depth_gt, mask.to(torch.bool))
            #     loss += silog_criterion.forward(lpg2x2, depth_gt, mask.to(torch.bool))
            #     loss += silog_criterion.forward(reduc1x1, depth_gt, mask.to(torch.bool))
            loss.backward()
            ...
            if global_step and global_step % args.log_freq == 0 and not model_just_loaded:
                ...
                print(print_string.format(args.gpu, examples_per_sec, loss.cpu().item(), var_sum.item(), var_sum.item()/var_cnt, time_sofar, training_time_left))

                if not args.multiprocessing_distributed or (args.multiprocessing_distributed
                                                            and args.rank % ngpus_per_node == 0):
                    writer.add_scalar('silog_loss', loss.cpu().item(), global_step)
                    writer.add_scalar('learning_rate', current_lr, global_step)
                    writer.add_scalar('var average', var_sum.cpu().item()/var_cnt, global_step)
            ...
            if step == steps_per_epoch-1:
                if not args.multiprocessing_distributed or (args.multiprocessing_distributed and args.rank % ngpus_per_node == 0):
                    ...
                    torch.save(checkpoint, args.log_directory + '/' + args.model_name + '/model')
