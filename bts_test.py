...
parser.add_argument('--model_ag',                   type=int,   help='whether to use attention gate',
                                            default=0)
...
def test(params):
    ...
    with torch.no_grad():
        for _, sample in enumerate(tqdm(dataloader.data)):
            ...
            # Predict
            if args.model_ag:
                lpg8x8, lpg4x4, lpg2x2, reduc1x1, depth_est, attn8x8, attn4x4, attn2x2, attn1x1= model(image, focal)
            else:
            ...
            if args.model_ag:
                attn_8x8s.append(attn8x8[0].cpu().numpy().squeeze())
                attn_4x4s.append(attn4x4[0].cpu().numpy().squeeze())
                attn_2x2s.append(attn2x2[0].cpu().numpy().squeeze())
                attn_1x1s.append(attn1x1[0].cpu().numpy().squeeze())
    ...
    for s in tqdm(range(num_test_samples)):
        ...
        if args.model_ag:
            attn_8x8 = attn_8x8s[s]
            attn_4x4 = attn_4x4s[s]
            attn_2x2 = attn_2x2s[s]
            attn_1x1 = attn_1x1s[s]
        ...
        if args.save_lpg:
            ...
            if args.dataset == 'nyu':
                ...
                if args.model_ag:
                    attn_1x1_cropped = attn_1x1[10:-1 - 9, 10:-1 - 9]
                    filename_lpg_cmap_png = filename_cmap_png.replace('.png', 'attn_1x1.png')
                    plt.imsave(filename_lpg_cmap_png, np.log10(attn_1x1_cropped), cmap='Greys')
                    attn_2x2_cropped = attn_2x2[10:-1 - 9, 10:-1 - 9]
                    filename_lpg_cmap_png = filename_cmap_png.replace('.png', 'attn_2x2.png')
                    plt.imsave(filename_lpg_cmap_png, np.log10(attn_2x2_cropped), cmap='Greys')
                    attn_4x4_cropped = attn_4x4[10:-1 - 9, 10:-1 - 9]
                    filename_lpg_cmap_png = filename_cmap_png.replace('.png', 'attn_4x4.png')
                    plt.imsave(filename_lpg_cmap_png, np.log10(attn_4x4_cropped), cmap='Greys')
                    attn_8x8_cropped = attn_8x8[10:-1 - 9, 10:-1 - 9]
                    filename_lpg_cmap_png = filename_cmap_png.replace('.png', 'attn_8x8.png')
                    plt.imsave(filename_lpg_cmap_png, np.log10(attn_8x8_cropped), cmap='Greys')
            else:
